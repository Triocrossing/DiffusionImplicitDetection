...
python src/eval.py \
  ...
  data.batch_size=512 \
  model=cfg_resnet \
  model.net._target_=src.models.components.resnet.resnet_34_cfg \
  model.net.num_classes=1 \
  tags="['CFG, eval']" \
  logger.wandb.name=EVAL_CFG_test \
  task_name=EVAL_CFG_test \
  ckpt_path=logs/resnet34_CFG_vs_Noise_SD1.4_concat2uncondb_bs512_0.5_newdata/runs/2024-03-21_10-35-39/checkpoints/last.ckpt \
