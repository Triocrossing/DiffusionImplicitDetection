...
python src/sweep.py \
  ...
  logger.wandb.name=CFG_vs_Noise_SD1.4_post_sweep_${seed} \
  task_name=CFG_vs_Noise_SD1.4_post_sweep_${seed} \
  seed=${seed} \
  model.optimizer.lr=0.0035 \
  model.net.dropout=0.23 \
  model.optimizer.weight_decay=0.015 \
