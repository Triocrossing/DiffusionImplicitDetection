...
class LaDDeModule(LightningModule):
    ...
    def forward(
        ...
        """
        return self.net(anchor), self.net(negative)
    ...
    def model_step(
        ...
        anchor_output, negative_output = self.forward(anchor=positive, negative=negative)

        # reshaping
        assert anchor_output.shape[0] % 2 == 0

        outputs = torch.cat([anchor_output, negative_output], dim=0)
        y = torch.cat([torch.zeros(anchor_output.shape[0]), torch.ones(negative_output.shape[0])]).to(anchor_output.device)
        
        indices = torch.randperm(outputs.size(0))
        outputs = outputs[indices]
        y = y[indices]
        logits = outputs.flatten()
                
        loss_cls = self.criterion_cls(logits, y.float())
        preds = outputs.sigmoid().flatten()
...
if __name__ == "__main__":
    _ = LaDDeModule(None, None, None, None)
